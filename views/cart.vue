<template>
    <div class="cart">
        ...
        <div class="cart-content">
            <div class="cart-content-main" v-for="(item, index) in productDicList">
                <div class="cart-info">
                ...
                <div class="cart-count">
                    <span class="cart-control-minus"
                          @click="handleCount(item.id,-1)">-</span>
                    {{ item.count }}
        ...
        <div class="cart-footer" v-show="cartList.length">
            <div class="cart-footer-desc">
                共计 <span>￥{{ costAll }}</span>
            </div>
            <div class="cart-footer-desc">
                应付总额 <span>￥{{ costAll - promotion }}</span>
                <br>
                <template v-if="promotion">（优惠 <span>￥{{ promotion }}</span>）</template>
            </div>
            <div class="cart-footer-desc">
                <div class="cart-control-order"
                     @click="handleOrder">现在结算
                </div>
            </div>
        </div>
    </div>
    ...
    export default {
        ...
        methods: {
            handleCount(id, count) {
                let editCart = this.cartList.find(item => item.id === id);
                (count > 0 || editCart.count > 1) && this.$store.commit('editCartCount', {id, count})
            },
            handleDelete(id) {
